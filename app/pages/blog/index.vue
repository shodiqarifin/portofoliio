    ...
    <div class="max-w-7xl mx-auto px-4 md:px-0">
      ...
      <div class="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div class="flex flex-wrap gap-2">
          <button class="rounded-full px-3 py-1 text-xs transition"
          ...
          <button v-for="tag in allTags" :key="tag" class="rounded-full px-3 py-1 text-xs transition"
      ...
      <div v-else class="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <NuxtLink v-for="post in filteredPosts" :key="post.path" :to="`/blog/${getSlug(post.path)}`"
